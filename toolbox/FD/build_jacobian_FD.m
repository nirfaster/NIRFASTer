function [J] = build_jacobian_FD(mesh,data,varargin)
...
if ~isreal(data.phi) && ~isreal(data.aphi)
    ...
    J.complex = conj([J_mus J_mua]);
        
    J.complete = zeros(size(J.complex).*[2 1]);
    % Scale to the Rytow approximatin
    % Extract log amplitude
    J.complete(1:2:end,:) = real(J.complex./data.complex);
    % Extract phase
    J.complete(2:2:end,:) = imag(J.complex./data.complex);
    
elseif isreal(data.phi) && isreal(data.aphi)
