function [data] = femdata_spec_TR_moments(mesh, varargin)
% FEMDATA_SPEC_TR_MOMENTS Returns spatial distributions and boundary values
%   of statistical moments or Mellin transforms of time-resolved data for
%   the 'spectral' FEM mesh in NIRFAST format. This function generates
...
%   time-resolved moments solver. Data calculated for all wavlengths as
%   specified in the input mesh structure.
%
% SYNTAX: 
%   [DATA] = FEMDATA_SPEC_TR_MOMENTS(MESH)
%   [DATA] = FEMDATA_SPEC_TR_MOMENTS(MESH, MAX_ORDER)
%   [DATA] = FEMDATA_SPEC_TR_MOMENTS(MESH, MAX_ORDER, 'field')
%   [DATA] = FEMDATA_STND_TR_MOMENTS(MESH, MAX_ORDER, 'field', 'mellin')
%   [DATA] = FEMDATA_SPEC_TR_MOMENTS(MESH, MAX_ORDER, 'field', 'mellin', SOLVER)
%   [DATA] = FEMDATA_SPEC_TR_MOMENTS(MESH, MAX_ORDER, 'field', 'mellin', OPTIONS)
...
%   MESH - FEM mesh in NIRFAST format. Spectral mesh (many wavelengths as
%   specified in 'MESH.wv' field). 
%   The output stucture DATA has following entries:
%     - DATA.phi [optional] is 4D matrix (n by m by l by k) of the spatial
%       distributions of the statistical moments where n - number of mesh
%       nodes, m - number of sources, l - number of wavelengths, k - number
%       of statistical moments requested. This value is optional and is
%       enabled by the optional input parameter 'field'. The moment unit
%       depends ont its order q, starting at the 0th order (q=0,1,2,...):
%       phot*s^q. 
%     - DATA.moments has size of (p by l by k) where p represents
%       source-detector combinations as specified in the 'MESH.link', l is
%       numer of wavelengths as specified in 'MESH.wv' and k holds the
%       requested moments. Moments units are the same as in the 'DATA.phi'.
...
% 
% WARNING: If the MESH.link specifies that a given wavelengths at a given
%          source-detecor pair is OFF, the DATA.moments values at that pair
%          will be NaN.
% 
% [DATA] = FEMDATA_SPEC_TR_MOMENTS(MESH, MAX_ORDER)
...
%   flight of photons) for sources, detectors and wavelengths as specified
%   in the MESH structure ('MESH.link' and 'MESH.wv').
...
%     - DATA.phi [optional] is 4D matrix (n by m by l by MAX_ORDER+1).
%     - DATA.moments is (p by l by MAX_ORDER+1).
% 
% [DATA] = FEMDATA_SPEC_TR_MOMENTS(MESH, MAX_ORDER, 'field')
%   Returns the optional 'DATA.phi' field with spatial distributions of the
%   statistical moments at all wavelengths.
% 
% [DATA] = FEMDATA_SPEC_TR_MOMENTS(MESH, MAX_ORDER, 'field', 'mellin')
...
% 
% [DATA] = FEMDATA_SPEC_TR_MOMENTS(MESH, MAX_ORDER, 'field', 'mellin', SOLVER) User
...
% 
% [DATA] = FEMDATA_SPEC_TR_MOMENTS(MESH, [], [], [], [], []) Optional fields
%   can be empty, e.g. it is valid to call this function as 
%   [DATA] = FEMDATA_SPEC_TR_MOMENTS(MESH, [], 'field', [], [], OPTIONS)
% 
% WARNING:
...
%          GET_FIELD_TR_MOMENTS_CUDA, FEMDATA_STND_TR_MOMENTS, GET_SOLVER,
...
% output boundary data declaration
data.moments = zeros(num_meas,nlambda,max_order+1);
...
for ind_wv = 1:nlambda
    ...
    for ind_order = 1:size(phi,3)
        data.moments(:,ind_wv,ind_order)= get_boundary_data(mesh,phi(:,:,ind_order));
    end
    mesh.link = link_copy;
