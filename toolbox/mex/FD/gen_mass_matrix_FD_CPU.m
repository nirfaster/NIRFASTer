% GEN_MASS_MATRIX_FD_CPU Builds sparse matrix representation of time-invariant
%   diffusion transport equation within FEM mesh in 2D (triangles) or 3D
%   (tetrahedrons). Parallelized for multithreading with OpenMP. Uses all
%   available CPU power.
% 
% SYNTAX:
%   [I_INDEX, J_INDEX, S_VALUE] = GEN_MASS_MATRIX_FD_CPU(MESH, F) 
%   [I_INDEX, J_INDEX, S_VALUE] = GEN_MASS_MATRIX_FD_CPU(NODES, ELEMENTS, ...
...
%   [I_INDEX, J_INDEX, S_VALUE] = GEN_MASS_MATRIX_FD_CPU(NODES, ELEMENTS, ...
%                                   BNDVTX, MUA, KAPPA, KSI, CM, F)
%       Works as GEN_MASS_MATRIX_FD_CPU(MESH,F). However, the input MESH
...
%       F - source frequency in Hz. Can be SINGLE or DOUBLE precision.
% 
% GEN_MASS_MATRIX_FD_CPU was tested to give symmetric A (I_INDEX, J_INDEX,
% S_VALUE) matrices. 
%
...
% is the reduced scattering coefficient in mm^-1, PHI(r,w) is the fluency
...
% GEN_MASS_MATRIX_FD_CPU transforms Eq. (1) into its matrix form of:
% 
%   A * PHI(r,w) = S(r)
% 
...
% 
% See also GEN_MASS_MATRIX_FD_CUDA, GEN_MASS_MATRIX_TR_CPU,
