% IntFG_tet4_CPU carries out a spatial convolutions of fields within a FEM
%   mesh. This function runs on parallel on the CPU threads and is
%   parallelized over the data pairs undergoing the convolution.
%
% SYNTAX:
%  [J_BORN] = IntFG_tet4_CPU(MESH,DATA_1,DATA_2)
%  [J_BORN] = IntFG_tet4_CPU(MESH,DATA_1,DATA_2,SOURCE_INDEX,DETECTOR_INDEX)
% 
% [J_BORN] = IntFG_tet4_CPU(MESH,DATA_1,DATA_2) MESH is a 2D or 3D FEM mesh
%   in NIRFAST format used to calculate DATA_1 and DATA_2.
...
%     combinations of spatial convolutions of fields DATA_1 and DATA_2.
%     Expressed in photons (photon packets to be exact) per second per
%     millimeter (phot*s^-1*mm^-1) if the input values are in photon
%     fluence rate values (phot*mm^-2s^-1 (3D) and phot*mm^-1s^-1 (2D)).
% 
% [J_BORN] = IntFG_tet4_CPU(MESH,DATA_1,DATA_2,SOURCE_INDEX,DETECTOR_INDEX)
...
%    fields DATA_1 and DATA_2 as indexed by SOURCE_INDEX and DETECTOR_INDEX,
%    E.g. convolved pairs of DATA_1 and DATA_2 [1 1;1 2;2 3;2 5].
% 
% HINT
%   #1 BORN approach
...
% 
% See also IntgradFgradG_tet4_CPU, BUILD_JACOBIAN
% 
%   MEX File function.
